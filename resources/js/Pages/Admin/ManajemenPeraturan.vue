...
<template>
  <AdminLayout title="Manajemen Peraturan">
    <div class="mx-auto max-w-5xl space-y-5">
      <div class="mb-6">
        ...
        <p class="mt-1 text-sm sm:text-base text-gray-500">
          Kelola dokumen peraturan yang ditampilkan di halaman publik
        </p>
      </div>

      <div class="bg-white rounded-xl shadow px-4 py-3 flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center">
        <div>
        ...
        <button
          ...
          class="bg-teal-600 text-white px-4 py-2 rounded-lg text-sm font-semibold hover:bg-teal-700 transition-all flex items-center gap-2 whitespace-nowrap self-start"
        >
      ...
      <div v-if="peraturans.length" class="grid gap-5 xl:grid-cols-2">
        <article
        ...
        >
          <div class="grid gap-0 md:grid-cols-[180px,1fr]">
            <div class="h-40 bg-slate-100 md:h-full">
              <img
                v-if="item.thumbnail"
              ...
              <div v-else class="flex h-full items-center justify-center px-4 text-center text-xs font-medium text-slate-400">
                Tidak ada thumbnail
              </div>
            </div>
            ...
            <div class="p-4">
              <div class="flex flex-col gap-2.5">
                <div>
                  <p class="text-[10px] font-semibold uppercase tracking-[0.16em] text-teal-600">
                    Dokumen Peraturan
                  </p>
                  <h3 class="mt-1.5 text-base sm:text-lg font-semibold text-slate-800">
                    {{ item.judul }}
                  </h3>
                </div>

                <div class="space-y-1 rounded-2xl bg-slate-50 p-2.5 text-xs sm:text-sm text-slate-600">
                ...
                <div class="flex flex-wrap gap-2">
                  <button
                    ...
                    class="inline-flex items-center rounded-lg bg-teal-600 px-3 py-1.5 text-xs sm:text-sm font-semibold text-white transition hover:bg-teal-700"
                  >
                    Lihat PDF
                  </button>
                  ...
                  <button
                    ...
                    class="inline-flex items-center rounded-lg border border-slate-300 px-3 py-1.5 text-xs sm:text-sm font-semibold text-slate-700 transition hover:bg-slate-100"
                  >
                    Edit
                  </button>
                  ...
                  <button
                    @click="destroy(item)"
                    class="inline-flex items-center rounded-lg border border-red-200 px-3 py-1.5 text-xs sm:text-sm font-semibold text-red-600 transition hover:bg-red-50"
                  >
                    Hapus
                  </button>
                </div>
      ...
      <div
        v-else
        class="rounded-xl border border-dashed border-slate-300 bg-white px-6 py-12 text-center text-slate-500"
      >
    ...
    >
      <div class="w-full max-w-lg rounded-xl bg-white p-5 sm:p-6 shadow-2xl">
        ...
        <div class="space-y-5">
          <div>
            ...
            <input
              ...
              class="w-full rounded-xl border border-slate-300 px-4 py-2.5 outline-none transition focus:border-teal-500 focus:ring-2 focus:ring-teal-200"
            />
          </div>
        ...
        <div class="mt-8 flex justify-end gap-3">
          <button
            @click="closeModal"
            class="rounded-lg border border-slate-300 px-5 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-100"
          >
            Batal
          </button>
    ...
    <div
      ...
      class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4 py-8"
    >
      <div class="w-full max-w-4xl h-[75vh] bg-white rounded-xl shadow-2xl flex flex-col">
        <!-- HEADER -->
